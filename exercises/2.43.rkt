        ...
        (filter
         ...
         (flatmap
          (lambda (rest-of-queens)
            (map (lambda (new-row)
                   (cond ((= 1 new-row) (newline) (display "k: ") (display k) (display " solution count: ") (display (length rest-of-queens))))
...
(define (timed-queen-test queen-function board-size)
  (let ((start-time (current-milliseconds))
        (solution (queen-function board-size))
        (end-time (current-milliseconds)))
    (display "elapsed time (ms): ")
    (- end-time start-time)))

;(timed-queen-test queens 8)
;(timed-queen-test queens-slow 8)
